...
- [x] **Recall UI ("What do you remember?")** — SHIPPED 2026-07-18 on branch
  ...
  17 new tests; suite green. Read-only v1 (no editing/deleting).
```
...
**Final state:** 11 commits on `recall-ui`, suite 20/20 (17 new recall tests),
reviews all green.

## Post-Review Addenda (final whole-implementation review)

6. **FALLBACK_MS = 800, not 400+400 as the spec phrased it** ("substring if
   semantic doesn't answer within 400ms after the debounce"). The plan chose a
   flat 800ms to give the embedding worker headroom on slow networks; the
   visible behavior is identical otherwise (substring shows at 800ms, a late
   semantic result still upgrades the list).
7. **Query-embedding accumulation capped (review catch).** `setQueryEmbedding`
   copies every completed recall query's vector into `m_queryEmbeds` (~6KB
   each, memory-only, never cleared) — a slow leak for a long-lived app. Fixed
   by `clearQueryEmbedding(key)` on delivery in `onQueryEmbeddingReady`, before
   the stale guard, so both consumed and dropped-stale keys are freed. Note:
   the reviewer's alternative (one shared `"recall-query"` key) was REJECTED —
   it would break the stale-key guard (an older query's result would pass
   `key == m_currentQueryKey` for the newer query's text).
