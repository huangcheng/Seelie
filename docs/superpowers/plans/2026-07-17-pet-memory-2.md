...
## Errata (applied during execution, 2026-07-17)

1. **Task 4 (pulled forward from Task 10):** `recordEpisode` returns `qint64` from the start (review fix) — Task 10 Step 1's signature change was unnecessary.
2. **Task 5:** `affection()` and `recentEpisodes()` are const (Task 3 review) — `memoryDigest()` calls them directly; the plan's `const_cast`s were removed. `setUserName` returns void, so its test call is a bare statement, not QVERIFY.
3. **Task 7 test vectors corrected:** the plan's `testRollupMergesNearDuplicates` filler vectors were all parallel (cos=1.0), which would have made dedup eat a filler; replaced with deterministic 16-dim pseudo-random fillers, and dup vectors made identical (guaranteed global max). The plan's `testDigestUsesSimilarityWhenEmbedded` had too few rows to exclude the irrelevant episode; 5 ranking fillers added.
4. **Task 8:** worker is top-level `EmbeddingWorker` (moc rejects nested Q_OBJECT); teardown is quit+wait-checked+delete; `LLMProfile::Protocol::OpenAIChat` scoped enum; baseUrl untrimmed per existing convention.
5. **Task 9:** memory `eventProcessed` connect placed in `setPersonaEngine` (last setter; constructor runs too early). Poke throttle placed once per handler (not duplicated per branch); extracted to `tryRecordPoke()` in Task 10.
6. **Task 7/8:** dedup comparison is `>=` (spec-exact "≥ 0.95").
