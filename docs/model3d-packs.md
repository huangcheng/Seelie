...
- **≤ 64 joints guideline.** OpenGL ES / WebGL clamp the skinning uniform array at 256 mat4s (≈ 64 joints × 2 matrices). Desktop GL usually allows more, but portable packs should stay under 64. **Mixamo rigs frequently exceed this** — they ship with finger, toe, and leaf bones you usually don't need. Remove them in Blender (Edit Mode → select bones → X) before exporting, or accept that some GL drivers will clamp the rig and animate with visible glitches. The converter warns at >64 joints; the runtime logs a similar warning at load time.
